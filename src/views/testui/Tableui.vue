  ...
  <div  class="app-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column width="110px" align="center" :label="'性别'">
        <template slot-scope="scope">
          <span>{{ getSexValue(scope.row.sex) }}</span>
          <!-- <span>{{ scope.row.sex | sexTypeFilter }}</span> -->
        </template>
      </el-table-column>
      <el-table-column width="80px" :label="'年龄'">
        <template slot-scope="scope">
...
export default {
  ...
  data() {
    return {
      ...
      sexOptions: [],
      sexKeyValue: {},
      sortOptions: [{ label: '升序', key: '+id' }, { label: '逆序', key: '-id' }],
      temp: {
        id: undefined,
        name: '姓名',
        sex: '1',
  ...
  filters: {
    sexTypeFilter(type) {
      console.log('此处有Bug,不知道如何访问到Vue对象!')
      return this.$data.sexKeyValue[type]
    }
  },
  created() {
    this.getDictionary()
  ...
  methods: {
    getSexValue(key) {
      return this.sexKeyValue[key]
    },
    getDictionary() {
      fetchDicList({ dicName: 'P_性别' }).then(response => {
        this.sexOptions = response.data
        this.sexKeyValue = this.sexOptions.reduce((acc, cur) => {
